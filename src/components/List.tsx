...
const List: FC = () => {
  const initialState: Omit<ITodo, '_id' | 'done'> = {
    name: '',
    description: '',
    cost: '',
  };
  ...
  const handleClickOnTitleEditButton = () => {
    setEditTitle(true);
  };
    ...
    <div>
      ...
      {!editTodo && (
        <div className='todos__container'>
          {todos?.length ? (
            <h3 className='list__items__title todos__container__title'>
              Todos on list: <span>'{list?.name}'</span>
            </h3>
          ) : (
            ''
          )}
          <Link to={'/lists'} className='list__items-back'>
            {'< back to lists'}
          </Link>
          {todos &&
            todos.map((todo) => (
              <div
                ...
                className={
                  todo.done
                    ? 'todo-item--done todo-item__container'
